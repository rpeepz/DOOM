...
void    launch_info(struct nk_context *ctx)
{
	...
	LAUNCH_W - (WINDOW_OFFSET * 2), LAUNCH_H / 5);
...
void	get_resolution(struct nk_context *ctx, int *window_width, int *window_height)
{
	...
	struct nk_rect size = nk_rect(WINDOW_OFFSET, (LAUNCH_H / 5) + (WINDOW_OFFSET * 2),
	LAUNCH_W - (WINDOW_OFFSET * 2), LAUNCH_H - (LAUNCH_H * 0.1667) - ((LAUNCH_H / 5) + (WINDOW_OFFSET * 3)));
	if (nk_begin(ctx, "resulution", size, NK_WINDOW_BORDER))
	{
		nk_layout_row_begin(ctx, NK_STATIC, 20, 2);
		nk_layout_row_push(ctx, 150);
		nk_label(ctx, "Show help at launch", NK_TEXT_LEFT);
		nk_layout_row_push(ctx, 40);
		nk_checkbox_label(ctx, "", &launch_help);
		nk_layout_row_end(ctx);
		nk_layout_row_dynamic(ctx, 20, 1);
		nk_label(ctx, "Select a resolution", NK_TEXT_CENTERED);
